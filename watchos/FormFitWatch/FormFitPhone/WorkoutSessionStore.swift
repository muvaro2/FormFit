...
enum WorkoutSessionImporter {
    static func hasImportedCSV(named sourceFilename: String, in context: ModelContext) throws -> Bool {
        let sessions = try context.fetch(FetchDescriptor<WorkoutSession>())
        return sessions.contains { $0.sourceFilename == sourceFilename }
    }
    ...
    static func importCSV(
        ...
        return try save(
            ...
            sampleRateHz: inferredSampleRate(from: activitySet),
            in: context
        )
    }
    ...
    static func save(
        ...
        if let sourceFilename {
            let existingSessions = try context.fetch(FetchDescriptor<WorkoutSession>())
                .filter { $0.sourceFilename == sourceFilename }
            for session in existingSessions {
                context.delete(session)
            }
        }
    ...
    private static func inferredSampleRate(from activitySet: ActivitySet) -> Double? {
